...
class GlazedTerracottaLike(val path: String, chinese: String, val mapColor: MapColor, val luminance: Int = 0) {
	...
	companion object {
		...
		val JIGSAW_PIECE_STONE = GlazedTerracottaLike("jigsaw_piece_stone", "拼图碎片石", JIGSAW.defaultMapColor).apply {
			if (ARRP) {
				ARRP_HELPER.pack.addRecipe_stoneCutting(JIGSAW_ARROW_STONE.fullBlock, fullBlock, 1)
			}
		}
		val JIGSAW_HALF_LINE_STONE = GlazedTerracottaLike("jigsaw_half_line_stone", "拼图半线石", JIGSAW.defaultMapColor).apply {
			if (ARRP) {
				ARRP_HELPER.pack.addRecipe_stoneCutting(JIGSAW_ARROW_STONE.fullBlock, fullBlock, 1)
			}
		}
		val JIGSAW_LINE_STONE = GlazedTerracottaLike("jigsaw_line_stone", "拼图直线石", JIGSAW.defaultMapColor).apply {
			if (ARRP) {
				ARRP_HELPER.pack.addRecipe_stoneCutting(JIGSAW_ARROW_STONE.fullBlock, fullBlock, 1)
			}
		}
